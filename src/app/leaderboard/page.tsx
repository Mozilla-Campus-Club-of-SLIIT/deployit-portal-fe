...
const RANK_STYLE: Record<
  number,
  { bg: string; text: string; label: React.ReactNode }
> = {
  0: {
    bg: "bg-yellow-400",
    text: "text-black",
    label: <TrophyIcon color="#000" />,
  },
  1: {
    bg: "bg-gray-300",
    text: "text-black",
    label: <MedalIcon color="#000" />,
  },
  2: {
    bg: "bg-amber-700",
    text: "text-white",
    label: <AwardIcon color="#fff" />,
  },
};
...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchAttempts = async () => {
      if (!user) return;
      try {
        const res = await fetch(`${API_URL}/api/attempts?userId=${user.uid}`, {
          headers: authHeaders(),
        });
        if (res.ok) {
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      ...
                      >
                        {searchQuery
                          ? "No Ninjas found matching your search."
                    ...
                    pagedLeaderboard.map((u, idx) => {
                      ...
                      return (
                        <tr
                          ...
                          style={{
                            borderBottom:
                              idx === pagedLeaderboard.length - 1
                                ? "none"
                                : "1px solid rgba(255, 255, 255, 0.05)",
                            background: isTop3
                              ? `rgba(245, 158, 11, ${0.03 - globalIndex * 0.01})`
                              : "transparent",
                            transition: "background 0.2s",
                          }}
                          onMouseEnter={(e) =>
                            (e.currentTarget.style.background = isTop3
                              ? `rgba(245, 158, 11, ${0.05 - globalIndex * 0.01})`
                        ...
                        >
                          <td
                            style={{ padding: "1.25rem 1.5rem", width: "80px" }}
                          >
                            <span
                              style={{
                                display: "flex",
                                ...
                                backgroundColor:
                                  rank?.bg === "bg-yellow-400"
                                    ? "#facc15"
                                    : rank?.bg === "bg-gray-300"
                                      ? "#d1d5db"
                                      : rank?.bg === "bg-amber-700"
                                        ? "#b45309"
                                        : undefined,
                              }}
                            >
