 ...
 */
async function uploadAvatarViaBackend(
  userId: string,
  file: File,
): Promise<string> {
  ...
  console.log(
    `[uploadAvatar] POSTing to ${API_URL}/api/upload-avatar — userId=${userId}, file=${file.name} (${file.size} bytes)`,
  );
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signup = async (
    ...
    if (photoFile) {
      try {
        ...
        console.warn(
          "[AuthContext] Photo upload failed, continuing without photo:",
          e,
        );
        photoUrl = undefined;
      }
  ...
  const resetPassword = async (email: string) => {
    try {
      ...
      console.warn(
        "[AuthContext] fallback for /api/forgot-password, mocking success.",
      );
      // mock success
